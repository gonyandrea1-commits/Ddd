const EXCEL_FILE = "brands.xlsx?v=20260422-1";

const fallbackBrands = [
  { id: 1, brand: "Frontier", category: "Internet", country: "USA", price: "", description: "Example brand. Replace data in brands.xlsx", link: "", active: "yes", sort: 1 },
  { id: 2, brand: "Eero", category: "Router", country: "USA", price: "", description: "Example device brand", link: "", active: "yes", sort: 2 }
];
...
function normalizeRow(row, index) {
  ...
  return {
    ...
    brand: obj.brand || obj.name || objназва || obj.бренд || "",
...
async function loadExcelFromServer() {
  ...
  if (!response.ok) throw new Error("brands.xlsx not found");
...
function parseWorkbook(buffer, source = "file") {
  ...
  brands = rows.map(normalizeRow).filter(item => item.brand && isActive(item.active)).sort((a, b) => a.sort - b.sort);
...
function loadFallback() {
  ...
  visibleBrands = brands;
  render();
  setStatus("brands.xlsx ще не знайдено. Показую тестові дані. Створи Excel через шаблон і завантаж у папку line.", "warn");
}
...
function bindEvents() {
  ...
  $("reloadBtn").addEventListener("click", async () => {
    setStatus("Оновлюю brands.xlsx...", "info");
